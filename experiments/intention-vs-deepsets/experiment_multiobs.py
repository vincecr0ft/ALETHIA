"""Multi-observable Intention head: the vertex-disclosure attack.

The supercharged experiment showed that m_ll-only encoders hit perfect
prediction R^2 but cannot disclose c_Hq^(3) or c_Hq^(1) regardless of
capacity, data, or PDF choice. The signal that breaks the vertex-operator
degeneracy lives in the cos(theta*) distribution, which is captured by
the lepton transverse-momentum spectrum.

This script trains an Intention head with two separate encoders
(psi_m, psi_pt) on a combined (m_ll, pT) context, then runs the linear-
probe identifiability test against CT18NNLO. If the multi-observable
lift works, vertex operators climb above the qualitative disclosure
threshold R^2 = 0.5.
"""
...
from intention_multiobs import IntentionFMMulti
from modules.surrogate.oracle_smeft import AnalyticSMEFTOracle


# Config (matches supercharged for fair compare)
PDF = "CT18NNLO"
D_PSI = 32
HIDDEN = 128
N_TRAIN = 500
...
Q_PT = 16
M_RANGE = (0.3, 2.3)        # TeV
PT_RANGE = (0.1, 1.0)       # TeV (lepton pT range, complementary to m_ll)
C_MAX_TRAIN = 0.7
C_OUTER = 1.0
N_META_STEPS = 3000
LR = 1e-3
...
def make_scenario_multi(oracle, c, rng):
    M_ctx = rng.uniform(*M_RANGE, size=K_M)
    PT_ctx = rng.uniform(*PT_RANGE, size=K_PT)
    M_q = rng.uniform(*M_RANGE, size=Q_M)
    PT_q = rng.uniform(*PT_RANGE, size=Q_PT)
    c_M = np.tile(c, (K_M, 1))
    c_PT = np.tile(c, (K_PT, 1))
    c_Mq = np.tile(c, (Q_M, 1))
    c_PTq = np.tile(c, (Q_PT, 1))
    Y_m_ctx  = oracle.truth(c_M, M_ctx)
    Y_pt_ctx = oracle.truth_pt(c_PT, PT_ctx)
    Y_m_q    = oracle.truth(c_Mq, M_q)
    Y_pt_q   = oracle.truth_pt(c_PTq, PT_q)
    return {
        "M_ctx": M_ctx.astype(np.float32),
        "Y_m_ctx": Y_m_ctx.astype(np.float32),
        "PT_ctx": PT_ctx.astype(np.float32),
        "Y_pt_ctx": Y_pt_ctx.astype(np.float32),
        "M_q": M_q.astype(np.float32),
        "Y_m_q": Y_m_q.astype(np.float32),
        "PT_q": PT_q.astype(np.float32),
        "Y_pt_q": Y_pt_q.astype(np.float32),
        "c": c.astype(np.float32),
    }


def make_dataset(n_scenarios, c_sampler, oracle, seed):
    rng = np.random.default_rng(seed)
    scenarios = []
    for _ in range(n_scenarios):
        c = c_sampler(rng, 1)[0]
        scenarios.append(make_scenario_multi(oracle, c, rng))
    return scenarios
...
def implicit_w_one(model, c, oracle, rng):
    ...
    PT_ctx = rng.uniform(*PT_RANGE, size=K_PT).astype(np.float32)
    Y_m  = oracle.truth(np.tile(c, (K_M, 1)),  M_ctx).astype(np.float32)
    Y_pt = oracle.truth_pt(np.tile(c, (K_PT, 1)), PT_ctx).astype(np.float32)
    _, w, _ = model.A_inv_and_w(M_ctx, Y_m, PT_ctx, Y_pt)
    return w


def run_probe(model, oracle, label="multi-obs"):
...
def main():
    print(f"# Build multi-observable datasets (PDF={PDF})")
    t0 = time.time()
    ...
    print(f"\n# Train multi-obs Intention (d_psi={D_PSI}, hidden={HIDDEN})")
    model = IntentionFMMulti(d_psi=D_PSI, hidden=HIDDEN, alpha=1e-3)
    print(f"  n_params = {model.n_params}")
    train(model, train_t, test_in_t)
    torch.save(model.state_dict(), OUT_DIR / "multiobs_model.pt")

    # Eval held-out
    with torch.no_grad():
        yp_m_in, yp_pt_in = model(test_in_t["M_ctx"], test_in_t["Y_m_ctx"],
                                  test_in_t["PT_ctx"], test_in_t["Y_pt_ctx"],
                                  test_in_t["M_q"], test_in_t["PT_q"])
        yp_m_out, yp_pt_out = model(test_out_t["M_ctx"], test_out_t["Y_m_ctx"],
    ...
    r2_pt_in  = r2_per_scenario(yp_pt_in.cpu().numpy(), test_in_t["Y_pt_q"].cpu().numpy())
    r2_m_out  = r2_per_scenario(yp_m_out.cpu().numpy(),  test_out_t["Y_m_q"].cpu().numpy())
    r2_pt_out = r2_per_scenario(yp_pt_out.cpu().numpy(), test_out_t["Y_pt_q"].cpu().numpy())

    print("\n# Held-out R^2")
    for label, r in (("m  in ", r2_m_in), ("m  out", r2_m_out),
                     ("pT in ", r2_pt_in), ("pT out", r2_pt_out)):
    ...
    summary = dict(
        config=dict(pdf=PDF, d_psi=D_PSI, hidden=HIDDEN, n_train=N_TRAIN,
                    K_m=K_M, K_pt=K_PT, n_params=model.n_params),
        held_out={
            "m_in_median":   float(np.median(r2_m_in)),
            "m_out_median":  float(np.median(r2_m_out)),
            "pt_in_median":  float(np.median(r2_pt_in)),
            "pt_out_median": float(np.median(r2_pt_out)),
        },
