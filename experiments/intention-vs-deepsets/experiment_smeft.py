"""End-to-end Intention-vs-DeepSets FM comparison against the analytic
SMEFT Drell-Yan oracle.

Identical pipeline to experiment.py but the ground-truth oracle is
`AnalyticSMEFTOracle` (closed-form LO Drell-Yan with PDF convolution),
not the polynomial toy. This is the headline benchmark of the paper.

Outputs land in experiments/intention-vs-deepsets/output_smeft/.
"""
...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_smeft")
...
def main():
    ...
    torch.save(int_model.state_dict(), os.path.join(OUTPUT_DIR, "intention_learned.pt"))
    torch.save(ds_model.state_dict(), os.path.join(OUTPUT_DIR, "deepsets.pt"))
    torch.save(reg_model.state_dict(), os.path.join(OUTPUT_DIR, "regressor.pt"))
    # Save scalar summary.
    summary = {
        "config": {
    ...
    print(f"\n# wrote {OUTPUT_DIR}/results.npz, summary.json, *.pt")
    return summary
